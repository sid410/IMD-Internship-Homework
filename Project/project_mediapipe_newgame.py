import cv2
import mediapipe as mp
import random
import time

mp_drawing = mp.solutions.drawing_utils
mp_drawing_styles = mp.solutions.drawing_styles
mp_hands = mp.solutions.hands

gestures = ["one", "two", "three", "four", "five"]  

def recognize_gesture(hand_landmarks): 
    tips = [4, 8, 12, 16, 20] 
    mcps = [2, 5, 9, 13, 17] 
    
    if hand_landmarks.landmark[8].y < hand_landmarks.landmark[5].y:
        open_one = True
    else:
        open_one = False
    
    if hand_landmarks.landmark[12].y < hand_landmarks.landmark[9].y:
                open_two = True
    else:
        open_two = False

    if hand_landmarks.landmark[16].y < hand_landmarks.landmark[13].y:
                open_three = True
    else:
        open_three = False

    if hand_landmarks.landmark[20].y < hand_landmarks.landmark[17].y:
                open_four = True
    else:
        open_four = False

    is_thumb_open = hand_landmarks.landmark[4].x > hand_landmarks.landmark[2].x  # 親指の判定
    
    if open_one == True and open_two == False and open_three == False and open_four == False and is_thumb_open == False:
        return "one"
    elif open_one == True and open_two == True and open_three == False and open_four == False and is_thumb_open == False:
        return "two"
    elif open_one == True and open_two == True and open_three == True and open_four == False and is_thumb_open == False:
        return "three"
    elif open_one == True and open_two == True and open_three == True and open_four == True and is_thumb_open == False:
        return "four"
    elif open_one == True and open_two == True and open_three == True and open_four == True and is_thumb_open == True:
         return "five"

def determine_winner(player, npc): #勝敗の判定
    if player == npc:
        return "Draw"
    elif ((player == "rock" and npc == "scissors") or 
         (player == "scissors" and npc == "paper") or 
         (player == "paper" and npc == "rock")):
        return "You Win!"
    else:
        return "You Lose!"

cap = cv2.VideoCapture(0)

with mp_hands.Hands(
    model_complexity=0,
    min_detection_confidence=0.5,
    min_tracking_confidence=0.5) as hands:
    
    game_active = False  
    countdown_start_time = None  
    npc_gesture = None  
    result = ""  
    saved_player_gesture = "None"
    draw_display_time = None

    while cap.isOpened():
        success, image = cap.read()

        image.flags.writeable = False
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        results = hands.process(image)

        image.flags.writeable = True
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        player_gesture = saved_player_gesture if npc_gesture is not None else "None"

        if results.multi_hand_landmarks and npc_gesture is None:
            for hand_landmarks in results.multi_hand_landmarks:
                mp_drawing.draw_landmarks(
                    image, hand_landmarks, mp_hands.HAND_CONNECTIONS,
                    mp_drawing_styles.get_default_hand_landmarks_style(),
                    mp_drawing_styles.get_default_hand_connections_style())

                player_gesture = recognize_gesture(hand_landmarks)
                saved_player_gesture = player_gesture  

        image = cv2.flip(image, 1)

        if not game_active:
            cv2.putText(image, "Press Enter to start", (150, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
            if cv2.waitKey(1) & 0xFF == 13: 
                game_active = True
                countdown_start_time = time.time()
                npc_gesture = None
                result = ""
                draw_display_time = None

        if game_active:
            elapsed_time = time.time() - countdown_start_time
            if npc_gesture is None: 
                if elapsed_time < 3: 
                    countdown_number = 3 - int(elapsed_time)
                    cv2.putText(image, str(countdown_number), (250, 250), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 255, 255), 5, cv2.LINE_AA)
                else:  
                    npc_gesture = random.choice(gestures)
                    result = determine_winner(saved_player_gesture, npc_gesture)
                    if result == "Draw":
                        draw_display_time = time.time()

        cv2.putText(image, f"Player: {saved_player_gesture}", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)

        if npc_gesture is not None:
            cv2.putText(image, f"NPC: {npc_gesture}", (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            cv2.putText(image, result, (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2, cv2.LINE_AA)

            if result == "Draw" and draw_display_time is not None:
                if time.time() - draw_display_time >= 1:
                    game_active = True
                    countdown_start_time = time.time()
                    npc_gesture = None
                    result = ""
                    draw_display_time = None
            else:
                cv2.putText(image, "Press Enter to play again", (10, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)

        cv2.imshow('teasobi', image)

        key = cv2.waitKey(1) & 0xFF
        if key == 27: 
            break
        elif key == 13 and npc_gesture is not None and result != "Draw": 
            game_active = False
            countdown_start_time = None
            npc_gesture = None
            result = ""

cap.release()
cv2.destroyAllWindows()
